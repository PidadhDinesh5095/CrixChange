...
export const verifyPayment = async (req, res) => {
  ...
  try {
    ...
    if (generated_signature === razorpay_signature) {
      ...
      try {
      await publishEvent(EVENT_TYPES.DEPOSIT_SUCCESS, user.id.toString(), { email: user.email, name: user.firstName, amount: amount });
    } catch (error) {
      console.error('Error sending deposit confirmation email:', error);
    }
    } else {
...
export const withdrawFunds = async (req, res) => {
  try {
    ...
    try {
      await publishEvent(EVENT_TYPES.WITHDRAWAL_COMPLETED, req.user.id.toString(), { email: req.user.email, name: req.user.firstName, amount: amount, status: 'completed' });
    }
    catch (error) {
