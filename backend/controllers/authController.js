...
export const register = async (req, res) => {
  ...
  try {
    ...
    await publishEvent(EVENT_TYPES.USER_REGISTERED, user._id.toString(), { email: user.email, name: user.firstName, verificationUrl: verificationUrl });
  } catch (error) {
...
export const changePassword = async (req, res) => {
  ...
  try {
    await publishEvent(EVENT_TYPES.PASSWORD_CHANGED, user._id.toString(), { email: user.email, changePasswordUrl: changePasswordUrl });
...
export const verifyEmail = async (req, res) => {
  ...
  try {
    await publishEvent(EVENT_TYPES.EMAIL_VERIFIED, user._id.toString(), { email: user.email, kycUrl: kycUrl });
  } catch (error) {
...
export const resendVerification = async (req, res) => {
  ...
  try {
    ...
    await publishEvent(EVENT_TYPES.EMAIL_VERIFY_REQUESTED, user._id.toString(), { email: user.email, name: user.firstName, verificationUrl: verificationUrl });
  } catch (error) {
    return res.status(500).json({
      success: false,
      message: 'Email could not be sent'
